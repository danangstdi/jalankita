...
export default async function page() {
  ...
  const endpoint = access !== 'ALL' ? `reports/status?access=${access}&reportStatus=PENDING` : 'reports/status?reportStatus=PENDING';
  ...
  return (
    <>
      ...
      <main className='m-2 flex justify-between items-center bg-slate-50 p-4 shadow-lg lg:ml-[19rem] lg:py-5'>
        <TableComponents api={`${process.env.NEXT_PUBLIC_BASEURL}/api/${endpoint}`} setReportStatus="Menunggu"/>
      </main>
