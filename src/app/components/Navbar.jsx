  ...
  return (
    <header className={`${isScrolled && 'bg-slate-800 shadow-lg'} z-40 p-4 fixed inset-x-0 top-0 md:px-6 md:py-2 lg:px-14`}>
      <nav className='flex justify-between items-center'>
        ...
        <button type='button' name='nav-button' onClick={() => setIsOpenNav(!isOpenNav)} className='w-11 h-11 flex items-center justify-center md:hidden'>
        ...
        <ul className={`${!isOpenNav ? 'hidden' : 'nav-list fixed bg-slate-800 shadow-lg inset-y-0 right-0 w-3/4 text-white z-50 px-4 md:static md:bg-transparent md:z-auto md:w-auto'} py-4 md:px-0 md:flex md:items-center md:gap-10`}>
          ...
          {listPageNav.map((nav) => (
            <li key={nav.link} className={`${pageNav == nav.label && 'font-semibold'} flex rounded-md text-white hover:bg-slate-700 hover:duration-200 md:hover:bg-transparent md:hover:text-gray-300`}>
